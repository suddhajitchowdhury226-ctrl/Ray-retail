...
const getImageUrl = (imagePath, baseUrl) => {
  ...
  const imageBase = import.meta.env.VITE_IMAGE_URL || 'https://api.rayonewholesale.com';
...
const MyOrderTab = ({ baseUrl, onOrderSelect, showModal, modalOrderId, setModalOrderData }) => {
  ...
  const getImageUrl = (imagePath) => {
    ...
    if (imagePath.startsWith('http')) return imagePath;
    const normalizedPath = imagePath.replace(/\\/g, '/').replace(/^\/+/, '');
    return `${import.meta.env.VITE_IMAGE_URL || 'https://api.rayonewholesale.com'}/${normalizedPath}`;
  };
